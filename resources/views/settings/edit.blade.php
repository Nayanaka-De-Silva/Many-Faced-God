...
<div class="container-fluid">
    ...
    <div class="row justify-content-center">
        <div class="col-md-8">
            ...
            {{-- Bank of Vivaldi Connection --}}
            <div class="card mb-4">
                <div class="card-header">
                    <h5 class="mb-0"><i class="bi bi-safe"></i> Loot Bank (Bank of Vivaldi)</h5>
                </div>
                <div class="card-body">
                    ...
                    <form action="{{ route('settings.update') }}" method="POST">
                        ...
                        <div class="mb-3">
                            <label for="vivaldi_base_url" class="form-label">
                                Server Base URL
                                @if($vivaldiHasDbOverride)
                                    <span class="badge bg-warning text-dark ms-1">DB override active</span>
                                @else
                                    <span class="badge bg-secondary ms-1">using config default</span>
                                @endif
                            ...
                            <input
                                type="url"
                                name="vivaldi_base_url"
                                id="vivaldi_base_url"
                                class="form-control @error('vivaldi_base_url') is-invalid @enderror"
                                value="{{ old('vivaldi_base_url', $vivaldiEffectiveUrl) }}"
                                placeholder="{{ $vivaldiConfigDefault }}"
                            >
                            @error('vivaldi_base_url')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                            <div class="form-text">
                                Leave empty to revert to the environment default
                                (<code>{{ $vivaldiConfigDefault }}</code>).
                            </div>
                        </div>
                        ...
                        <div class="d-flex gap-2 align-items-center flex-wrap">
                            ...
                            <button type="button" id="testVivaldiBtn" class="btn btn-outline-info">
                                <i class="bi bi-plug"></i> Test Connection
                            </button>
                            <span id="testVivaldiResult" class="ms-2 small" aria-live="polite"></span>
                        </div>
                    </form>
...
<script>
    function wireConnectionTest(buttonId, resultId, url) {
    ...
    wireConnectionTest('testConnectionBtn', 'testConnectionResult', '{{ route('settings.test-spell-library') }}');
    wireConnectionTest('testVivaldiBtn', 'testVivaldiResult', '{{ route('settings.test-vivaldi') }}');
</script>
@endsection
